...
function getCustomServiceAddress(service) {
    ...
    if (service._subType === 'luckmail') {
        ...
        const preferExistingAccountList = service.config?.prefer_existing_account_list !== false;
        ...
        const accountMode = `<div style="color: var(--text-muted); margin-top: 4px;">现有账号优先：${preferExistingAccountList ? '开启' : '关闭'}</div>`;
        const accountSummary = service.config?.has_account_list
            ...
            : '';
        return `${escapeHtml(baseUrl)}<div style="color: var(--text-muted); margin-top: 4px;">${detail}</div>${accountMode}${accountSummary}`;
    }
    const baseUrl = service.config?.base_url || '-';
...
async function handleAddCustom(e) {
    ...
    if (subType === 'moemail') {
        ...
        config = {
            ...
            prefer_existing_account_list: formData.get('lm_prefer_existing_account_list') === 'on',
...
async function editCustomService(id, subType) {
    try {
        ...
        if (resolvedSubType === 'moemail') {
            ...
            document.getElementById('edit-lm-prefer-existing-account-list').checked = service.config?.prefer_existing_account_list !== false;
